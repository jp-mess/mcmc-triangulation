...
def plot_cov_ellipses(covariances, nstd=1, ax=None):
...
def plot_one():
  ...
  chosen = data_list[0]
  ax = plot_cov_ellipse(chosen["variance"], chosen["mean"], nstd=1)
  plt.show()
...
def plot_two():
  ...
  small_data = "small_covariances.txt"
  small_data = read_means_and_covariances(small_data)[0]

  large_data = "large_covariances.txt"
  large_data = read_means_and_covariances(large_data)[0]  

  ax = plot_cov_ellipses([small_data["variance"], large_data["variance"]])
  plt.show()
